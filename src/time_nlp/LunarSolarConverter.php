 ...
 * From Python3 version of Time_NLP
 * @package time_nlp
 */
class LunarSolarConverter
{
    ...
    #####################################################################################
    public $lunar_month_days = [1887, 0x1694, 0x16aa, 0x4ad5, 0xab6, 0xc4b7, 0x4ae, 0xa56, 0xb52a,
    ...
    #  离元旦多少天  春节月  春节日
    #####################################################################################
    public $solar_1_1 = [1887, 0xec04c, 0xec23f, 0xec435, 0xec649, 0xec83e, 0xeca51, 0xecc46, 0xece3a,
    ...
    public function getBitInt($data, $length, $shift) {
        return ($data & (((1 << $length) - 1) << $shift)) >> $shift;
    }

    public function lunarToSolar(Lunar $lunar) {
        $days = $this->lunar_month_days[$lunar->lunar_year - $this->lunar_month_days[0]];
        $leap = $this->getBitInt($days, 4, 13);
        $offset = 0;
        $loop_end = $leap;
        if (!$lunar->is_leap) {
            if ($lunar->lunar_month <= $leap || $leap == 0) {
                $loop_end = $lunar->lunar_month - 1;
            } else {
                $loop_end = $lunar->lunar_month;
            }
        }
        ...
        $solar11 = $this->solar_1_1[$lunar->lunar_year - $this->solar_1_1[0]];

        $y = $this->getBitInt($solar11, 12, 9);
        $m = $this->getBitInt($solar11, 4, 5);
        $d = $this->getBitInt($solar11, 5, 0);

        return $this->solarFromInt($this->solarToInt($y, $m, $d) + $offset - 1);
    }

    public function solarFromInt($g) {
        $y = intval((10000 * $g + 14780) / 3652425);
        $ddd = $g - (365 * $y + intval($y / 4) - intval($y / 100) + intval($y / 400));
        if ($ddd < 0) {
            $y -= 1;
            $ddd = $g - (365 * $y + intval($y / 4) - intval($y / 100) + intval($y / 400));
        }
        ...
        $mm = ($mi + 2) % 12 + 1;
        $y += intval(($mi + 2) / 12);
        $dd = $ddd - intval(($mi * 306 + 5) / 10) + 1;
        $solar = new Solar($y, $mm, $dd);
        return $solar;
    }

    public function solarToInt($y, $m, $d) {
        $m = ($m + 9) % 12;
        $y -= intval($m / 10);
        return 365 * $y + intval($y / 4) - intval($y / 100) + intval($y / 400) + intval(($m * 306 + 5) / 10) + ($d - 1);
    }
}